...
world.afterEvents.entityHurt.subscribe(data => {
  ...
  if (damageSource.damagingEntity && damageSource.damagingEntity.typeId == "minecraft:player" && damageSource.cause == EntityDamageCause.entityAttack) {
    if (damageSource.damagingEntity.getProperty("morphing_bracelet:entity") == 15) { hurtEntity.addEffect("wither", 200); };
    if (damageSource.damagingEntity.getProperty("morphing_bracelet:entity") == 19) { hurtEntity.addEffect("hunger", 600); };
    if (damageSource.damagingEntity.getProperty("morphing_bracelet:entity") == 33) { hurtEntity.addEffect("poison", 140); };
    if (damageSource.damagingEntity.getProperty("morphing_bracelet:entity") == 40 && damageSource.damagingEntity.getComponent("minecraft:mark_variant").value == 0) {
